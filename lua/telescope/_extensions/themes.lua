...
-- Custom theme picker
-- uses custom_theme global variable
...
local function theme_switcher(opts)
   ...
   local local_utils = require "modules.themes"
   ...
   if next(themes) ~= nil then
      ...
      local current_theme = vim.g.custom_theme
      local new_theme = ""
      local change = false
      ...
      picker.close_windows = function(status)
         ...
         -- set custom_theme global var
         vim.g.custom_theme = final_theme
      end
      -- launch the telescope picker
      picker:find()
   else
      print("No themes found in " .. vim.fn.stdpath "config" .. "/lua/colors/themes")
   end
end
