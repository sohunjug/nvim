...
function autocmd.load_autocmds()
  local definitions = {
    --[[ packer = {
      {"BufWritePost","*.lua","lua require('core.plugins').auto_compile()"};
    }, ]]
    bufs = {
      ...
      {"BufWritePre","*.go","lua require('go.format').gofmt()"};
      {"BufWritePre","*.lua","lua vim.lsp.buf.formatting()"};
      {"BufWritePre","*.go","lua require('go.format').goimport()"};
    };
    term = {
      { "TermOpen", "term://*", "setlocal nonumber norelativenumber | setfiletype terminal" };
    };
